...
export default class Nominees extends Component {
  ...
  displayNoms = () => {
    ...
    if (this.props.search === true) {
      path =
        "https://fast-cove-41298.herokuapp.com/search?tags=" +
        this.props.tags.replace(/ /g, "%20") +
        "&page=" +
        (this.state.page + 1);
    } else {
      path =
        "https://fast-cove-41298.herokuapp.com/paged_nominations?page=" +
        (this.state.page + 1);
    }
    this.setState({
      persons: this.state.next_persons,
    ...
    axios
      ...
      .then(res => {
        ...
        this.setState({
          showNoms: true,
          next_persons: [...this.state.persons, ...newPersons]
        });
      })
      ...
      <div className="pageBody">
        {this.state.persons.map((x, i) => (
          <Nominee
            ...
            name={this.state.persons.slice(i, i + 1).map(person => (
              <p>{person.name}</p>
            ))}
            description={this.state.persons.slice(i, i + 1).map(person => (
              <p>{person.description}</p>
            ))}
            duas={this.state.persons.slice(i, i + 1).map(person => (
              <p>{person.duas}</p>
            ))}
            id={this.state.persons.slice(i, i + 1).map(person => (
              <p>{person.id}</p>
            ))}
            image={this.state.persons.slice(i, i + 1).map(person => (
              <p>{person.image}</p>
            ))}
            country={this.state.persons.slice(i, i + 1).map(person => (
              <p>{person.country}</p>
            ))}
          />
        ))}
        <div>
          {this.state.showNoms ? (
            <RestOfNoms
              ...
              isLoggedIn={this.props.isLoggedIn}
              promptForLogin={this.promptForLogin}
              nominees={this.state.persons.slice(
                18 * (this.state.page - 1),
                this.state.persons.length
              )}
            />
          ) : null}
        </div>
        <div onClick={this.displayNoms}>{this.displayViewButton()}</div>
      </div>
    );
  }
}
      ...
      <div className="pageBody">
        {this.props.nominees.map((x, i) => (
          <Nominee
            ...
            image={this.props.nominees.slice(i, i + 1).map(person => (
              <p>{person.image}</p>
            ))}
            name={this.props.nominees.slice(i, i + 1).map(person => (
              <p>{person.name}</p>
            ))}
            description={this.props.nominees.slice(i, i + 1).map(person => (
              <p>{person.description}</p>
            ))}
            duas={this.props.nominees.slice(i, i + 1).map(person => (
              <p>{person.duas}</p>
            ))}
            id={this.props.nominees.slice(i, i + 1).map(person => (
              <p>{person.id}</p>
            ))}
            country={this.props.nominees.slice(i, i + 1).map(person => (
              <p>{person.country}</p>
            ))}
          />
        ))}
...
class Nominee extends Component {
  ...
  fetchCommentList = () => {
    return (
      <p>
        <b>Shehryar Assad: </b> Awesome stoof
      </p>
    );
  };
      ...
      <div className="columns" style={{ position: "relative" }}>
        <ul className="person">
          ...
          <div className="person content">
            ...
            <Well bsSize="large" className="well">
              {this.props.description}
            </Well>
          </div>
          <li className="dua">
            <Button onClick={this.toggleCommentModal}>Comment</Button>

            <Modal
              show={this.state.commentModalOpen}
            ...
            >
              ...
              <Modal.Footer>
                <Button onClick={this.toggleCommentModal}>Close</Button>
              </Modal.Footer>
            </Modal>
